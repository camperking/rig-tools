...
impl Tool for BashTool {
    ...
    async fn definition(&self, _prompt: String) -> ToolDefinition {
        ToolDefinition {
            name: "bash".to_string(),
            description: t!("bash.description").to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "command": {
                        "type": "string",
                        "description": t!("bash.param_command").to_string()
                    },
                    "timeout": {
                        "type": "integer",
                        "description": t!("bash.param_timeout").to_string()
                    }
                },
                "required": ["command"]
            }),
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        match result {
            ...
            Ok(Err(e)) => Err(ToolError(
                t!("bash.error_execute_failed", error = e.to_string()).to_string(),
            )),
            Err(_) => Err(ToolError(
                t!("bash.error_timeout", timeout = timeout_secs).to_string(),
            )),
        }
    }
}
